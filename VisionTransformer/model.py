...
class LinearProjection(nn.Module):
    def __init__(self, patch_vec_size, num_patches, latent_vec_dim, drop_rate):
        super().__init__()
        self.linear_proj = nn.Linear(patch_vec_size, latent_vec_dim)   # P^2 * C, d
        self.cls_token = nn.Parameter(torch.randn(1, latent_vec_dim))  # d  (learnable)
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches+1, latent_vec_dim))  # 1*(N+1)*d  => 1*(H*W+1)*d
...
class MultiheadedSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = torch.softmax(q @ k / self.scale, dim=-1)  # @ 매트릭스 곱
...
class VisionTransformer(nn.Module):
    def __init__(self, patch_vec_size, num_patches, latent_vec_dim, num_heads, mlp_hidden_dim, drop_rate, num_layers, num_classes):
        super().__init__()
        self.patchembedding = LinearProjection(patch_vec_size=patch_vec_size, num_patches=num_patches, # P^2 * C, H*W/P^2
                                               latent_vec_dim=latent_vec_dim, drop_rate=drop_rate)     # d, H*W+1
    ...
    def forward(self, x):
        att_list = []   # 각 레이어의 attention map 저장
